LimpiarPedbanda = function(playerPed)
...
AbrirArmariobanda = function()
    ...
    local elements = {
		...
		--{label = "Ropa de trabajo", value = "bandaclothes"},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armario', {
		...
		if data.current.value == "locker" then
			...
			exports['linden_inventory']:OpenStash({ name = "Locker-banda", owner = true, slots = 70})
			Citizen.Wait(850)
...
RopaServicioPolicia = function()
    ...
    ESX.TriggerServerCallback('masterjob:server:sex', function(sex)
		...
		if gender == "m" then
			...
			if ESX.PlayerData.job.grade_name == "policia6" or ESX.PlayerData.job.grade_name == "policia7" or ESX.PlayerData.job.grade_name == "boss" then
